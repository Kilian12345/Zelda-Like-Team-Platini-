Shader "Unlit/Glitch"
{
    Properties
    {
        ...
        //Function
        _ValueOne ("_ValueOne Traits/Grain", Int) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float random1(float p) { return frac(sin(p * _ROneSin) * _ROneFract) * 1.5 - 0.0; }
            ...
            fixed4 frag (v2f_img i) : COLOR
            {
                ...
                p.x += lerp(random3(float3(p.x - fmod(p.x + _ValueOne,random1(p.y * _ValueTwo)), p.y, pow(p.y,5))), 0.0, _ValueFour + exp(_ValueFive * random1(_Time - fmod(_Time, _ValueSix))));
